...
export function Title({ match,history, title, bg, isImg }) {
    return (
        <div className='relative z-0 '>
            ...
            <div className='whitespace-nowrap hidden md:hidden lg:visible absolute lg:left-[25%] md:left-[35%] sm:left-[45%] top-[14vmin] w-[60%] lg:flex flex-row justify-between items-center'>
                ...
                <div className=' sm:hidden md:hidden lg:flex flex-row justify-center items-center  font-light text-[0.75em]  cursor-pointer tracking-[0.22em]'>
                    <a className={`text-[#fff] mr-[8vmin] hover:text-[#231f20] font-Common `} onClick={() => { history.push('/') }}>ANA SAYFA</a>
                    <a className={`text-[#fff] mr-[8vmin] hover:text-[#231f20] font-Common`} onClick={() => { history.push('/AboutUs') }}>BİZ KİMİZ?</a>
                    <a className={`text-[#fff] mr-[8vmin] hover:text-[#231f20] font-Common`} onClick={() => { history.push('/Catalogs') }}>KATALOGLARIMIZ</a>
                    <a className={`text-[#fff] mr-[8vmin] hover:text-[#231f20] font-Common`} onClick={() => { history.push('/Products') }}>ÜRÜNLERİMİZ</a>
                    <a className={`text-[#fff] mr-[8vmin] hover:text-[#231f20] font-Common`} onClick={() => { history.push('/Contacts') }}>İLETİŞİM</a>
                </div>
            </div>
            ...
            {isImg ? <img className='w-[100vw]  h-[60vh] left-0 top-0 z-10 bg-repeat' src={bg} alt=""></img> :
                <div className='grandyantbg w-full  h-[60vh] left-0 top-0 z-10 ' />
            }
        </div>
    )
}
